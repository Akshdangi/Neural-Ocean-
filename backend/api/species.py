...
async def train_model(epochs: int = 5, batch_size: int = 16):
    """Train the species identification model."""
    try:
        metrics = service.train(epochs=epochs, batch_size=batch_size)
        return TrainResponse(
            status="success",
            message="Species model trained successfully",
            metrics=metrics,
